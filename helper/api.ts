...
export default class Req {
        ...
        .then(() => {
          if (!this.notion) {
            logToRenderer(
              "error",
              cn
                ? "[Notion] Notion API Token 未配置"
                : "[Notion] Notion ingetration token not config"
            )
          ...
          return new Promise((res, rej) => {
            setTimeout(() => {
              res(
                collectPaginatedAPI<{ [key: string]: string }, any>(
                  ...
                  logToRenderer(
                    "error",
                    cn
                      ? "[Notion] collectPaginatedAPI 接口发生错误:"
                      : "[Notion] collectPaginatedAPI API error:",
                    err
                  )
                  rej(null)
                })
        ...
        .catch((err) => {
          if (err) {
            logToRenderer(
              "error",
              cn
                ? "[Notion Flow] 接口初始化过程发生错误:"
                : "[Notion Flow] Init API error:",
              err
            )
          }
          return Promise.reject(null)
  ...
  async uploadNotionImageToOSS(props: {
    url: string
    meta: Meta
    id: string
    debug: boolean
    ...
    const { language: lang, oss } =
      (await storage.get<PublisherOptions>("options")) || {}
    const cn = lang === "cn"
    try {
      ...
      if (!this.oss) {
        logToRenderer(
          "warn",
          cn
            ? "[OSS] OSS 服务未配置，媒体文件将会被忽略"
            : "[OSS] OSS service not config, media will be ignored"
        )
      ...
      if (!cdn) {
        logToRenderer(
          "warn",
          cn
            ? "[OSS] CDN 地址未配置，媒体文件将会被忽略"
            : "[OSS] CDN address not config, media will be ignored"
        )
        return Promise.resolve(null)
      }
      ...
      if (!mediaPath) {
        logToRenderer(
          "warn",
          cn
            ? "[OSS] 媒体上传路径未配置，媒体文件将会被忽略"
            : "[OSS] Media path not config, media will be ignored"
        )
        return Promise.resolve(null)
      }
      ...
      const key = `${cosPath}/${uuid ? btoa(uuid) + "-" : ""}${id}.${
        suffix && hasSuffix ? suffix : "webp"
      }`
      try {
          ...
          if (res.status === 200) {
            ...
            try {
              ...
              if (!debug) {
                logToRenderer(
                  "info",
                  cn ? `[OSS] 准备上传 ${id}:` : `[OSS] Ready to upload ${id}:`,
                  { key, cdn, full: uri }
                )
              ...
              logToRenderer(
                "error",
                cn ? `[OSS] 上传 ${id} 出错:` : `[OSS] Upload ${id} faild:`,
                err
              )
              throw new Error(err)
            }
            ...
            logToRenderer(
              "error",
              cn
                ? "[Notion] 获取 Notion 图片错误:"
                : "[Notion] Get Notion image error:",
              res
            )
            throw new Error(err)
          }
      ...
      logToRenderer(
        "error",
        cn ? "[OSS] 上传媒体过程中发生错误:" : `[OSS] Try upload media err:`,
        err
      )
      throw new Error(err)
    }
          ...
          return this.notion.pages
            ...
            .then((response) => {
              ...
              if (cover && publisher?.["headerImgName"]) {
                ...
                return this.uploadNotionImageToOSS({
                  url: cover,
                  meta,
                  id: blockId,
                  debug,
                  ...
                  .then((coverUrl) => {
                    if (!coverUrl) {
                      ...
                      logToRenderer(
                        "warn",
                        cn
                          ? "[OSS] OSS 服务未配置，头图将会被忽略："
                          : "[OSS] OSS service not config, cover image will be ignored:"
                      )
                    } else {
                  ...
                  .catch((err) => {
                    logToRenderer(
                      "error",
                      cn
                        ? "[OSS] 上传头图到 OSS 出错, 忽略该属性:"
                        : "[OSS] Upload cover to OSS error, ignore it:",
                      err
                    )
                    Object.keys(meta).forEach((key) => {
                      if (!meta[key]) {
            ...
            .catch((err) => {
              logToRenderer(
                "error",
                cn
                  ? "[Notion] Notion pages.retrieve 接口报错:"
                  : "[Notion] Notion pages.retrieve API error:",
                err
              )
              return Promise.reject(null)
            })
      ...
      return storage.get<PublisherOptions>("options").then((options) => {
        ...
        if (!this.github) {
          logToRenderer(
            "error",
            cn
              ? "[Github] Github Personal Token 未配置"
              : "[Github] Github Personal Token not config"
          )
          return Promise.reject(null)
        }
        if (!publisher) {
        ...
        if (!_path) {
          logToRenderer(
            "error",
            cn
              ? "[Github] Github 文件上传路径未配置或不可用"
              : "[Github] File path not config or invalid"
          )
          return Promise.reject(null)
        }
        ...
        return this.github.rest.repos
          ...
          .then((res): any => {
            ...
            logToRenderer(
              "info",
              cn
                ? "[Github] Github 指定文件已存在，即将更新，文件信息:"
                : "[Github] File exist, update it now, file info:",
              _res
            )
            ...
            if (debug) {
              logToRenderer(
                "info",
                cn
                  ? "[Debug] createOrUpdateFileContents 方法调用，配置为:"
                  : "[Debug] Call function createOrUpdateFileContents, params is:",
                createOrUpdateConfig
              )
              return cn
                ? `[Debug] Github 文件「更新」成功`
                : `[Debug] Github file update success`
            } else {
              return this.github.rest.repos
                .createOrUpdateFileContents(createOrUpdateConfig)
                .then((res) => {
                  logToRenderer(
                    "info",
                    cn
                      ? "[Github] Github 更新文件成功:"
                      : "[Github] Update file success:",
                    res
                  )
                  return res
                })
          ...
          .catch((err): any => {
            if (err?.status === 404) {
              logToRenderer(
                "info",
                cn
                  ? "[Github] Github 文件不存在，即将新建，路径为:"
                  : "[Github] Github file not exist, create one now, path is:",
                _path
              )
              const createOrUpdateConfig = {
                ...this.githubCommon,
              ...
              if (debug) {
                ...
                return cn
                  ? `[Debug] Github 文件「创建」成功`
                  : `[Debug] Github file create success`
              } else {
                return this.github.rest.repos
                  .createOrUpdateFileContents(createOrUpdateConfig)
                  .then((res) => {
                    logToRenderer(
                      "info",
                      cn
                        ? "[Github] Github 文件创建成功:"
                        : "[Github] Create file success:",
                      res
                    )
                    return res
                  })
            ...
            oss
              ...
              .then((data) => {
                logToRenderer(
                  "info",
                  cn
                    ? `[OSS ${endpoint ? "自建" : "AWS"}] 文件已存在 ${key}:`
                    : `[OSS ${
                        endpoint ? "Self-Host" : "AWS"
                      }] File exist ${key}:`,
                  data
                )
                res(data)
            ...
            oss
              ...
              .then((data) => {
                ...
                logToRenderer(
                  "info",
                  cn
                    ? `[OSS ${endpoint ? "自建" : "AWS"}] 上传文件成功 ${key}:`
                    : `[OSS ${
                        endpoint ? "Self-Host" : "AWS"
                      }] Upload success ${key}:`,
                  data
