...
import reduxStore, { setAigc, setLogs, setPublisher } from "$store"
import type {
  AigcData,
  Meta,
  PublisherConfig,
...
const getPublisherConfig = async (storage) => {
  ...
  logToRenderer(
    "info",
    options?.language === "cn"
      ? "[Notion Flow] 获取发布 tab 配置:"
      : "[Notion Flow] Get publisher config:",
    config
  )
}
...
const getAigcConfig = async (storage) => {
  ...
  logToRenderer(
    "info",
    options?.language === "cn"
      ? "[Notion Flow] 获取 AIGC 配置:"
      : "[Notion Flow] Get aigc config:",
    aigc
  )
  if (!aigc || !aigc.model) {
...
const notionGetToc = (
  cb: (type: "toc" | "selection", toc: TocItem[] | string) => void
) => {
  const toc: TocItem[] = []
...
const notionMutation = (
  cb: (type: "toc" | "selection", arg: TocItem[] | string) => void
) => {
  ...
  const eventCb = (e) => {
    ...
    timer = setTimeout(() => {
      if (window.getSelection()?.type === "None") {
        // Note: 此时判断是否为块级选中
        const selectBlock = [
          ...document.querySelectorAll(".notion-selectable-halo")
        ]
        if (selectBlock.length) {
          // console.log('selectBlock:', selectBlock, target);
          if (
            selectBlock.every((ele) => {
              return AIGC_BLOCKS.some(
                (type) =>
                  ele.parentElement?.classList.contains(`notion-${type}-block`)
              )
            })
          ) {
            // Note: 获取 block 选中的内容
...
const notionLocateHeading = (
  key: string,
  cb: (prop: { type: "fail" | "succ"; error?: Error }) => void
) => {
  try {
    document
      .querySelector(".notion-page-content")
      ?.querySelector(`[data-block-id="${key}"]`)
...
const _inline = (list: any): string => {
  ...
  return list.reduce((prev: any, curr: any) => {
    const annotations = Object.keys(curr?.annotations || []).reduce(
      (prev, style) => {
...
const notion2markdown = async function (
  this: Req,
  list: any,
  meta: Meta,
  indent: number,
  ...
  return storage.get<PublisherOptions>("options").then((props) => {
    ...
    if (!Array.isArray(list)) {
      logToRenderer(
        "error",
        cn
          ? "[Notion] Notion 子元素需要是个数组:"
          : "[Notion] Notion child need to be an array:",
        list
      )
          ...
          return (async () => {
            ...
            switch (type) {
                  ...
                  _toContent("notion-bookmark-desc-get", id, (props) => {
                    ...
                    if (title || desc || img) {
                      ...
                      logToRenderer(
                        "info",
                        cn
                          ? "[Notion Flow] 从页面中获取 Bookmark 信息错误，将使用默认链接格式"
                          : "[Notion Flow] Get info from page error, use default bookmark style"
                      )
                        ...
                        res(
                          tableRows.reduce(
                            (prev: any, curr: any, index: number) => {
                              ...
                              let divider = ""
                              const cellsString = cells
                                .map((cell: any, key: number) => {
              ...
              case "to_do": {
                ...
                try {
                  return `${Array.from({ length: indent * 4 })
                    .fill(" ")
                    .join("")}${isChecked ? "- [x] " : "- [ ] "}${_inline(
                    block.rich_text
                  )}\n${(
                    await notion2markdown.bind(this)(
                      children,
                      meta,
                      ++_indent,
                      debug
                  ...
                  logToRenderer(
                    "error",
                    cn
                      ? "[Notion Flow] to_do to markdown 发生错误:"
                      : "[Notion Flow] Error happen when to_do to markdown:",
                    e
                  )
                  return Promise.reject(null)
                }
              ...
              case "numbered_list_item": {
                ...
                try {
                  return `${Array.from({ length: indent * 4 })
                    .fill(" ")
                    .join("")}1. ${_inline(block.rich_text)}\n${(
                    await notion2markdown.bind(this)(
                      children,
                      meta,
                      ++_indent,
                  ...
                  logToRenderer(
                    "error",
                    cn
                      ? "[Notion Flow] numbered_list_item to markdown 发生错误:"
                      : "[Notion Flow] Error happen when numbered_list_item to markdown:",
                    e
                  )
                  return Promise.reject(null)
                }
              ...
              case "bulleted_list_item": {
                ...
                try {
                  ...
                  logToRenderer(
                    "error",
                    cn
                      ? "[Notion Flow] bulleted_list_item to markdown 发生错误:"
                      : "[Notion Flow] Error happen when bulleted_list_item to markdown:",
                    e
                  )
                  return Promise.reject(null)
                }
              ...
              case "video": {
                ...
                if (_url) {
                  if (block.type === "file") {
                    // Note: 获取文件后缀
                    try {
                      const _ossUrl = await this.uploadNotionImageToOSS({
                        url: _url,
                        meta,
                        id: item.id,
                        debug
                      ...
                      logToRenderer(
                        "error",
                        cn
                          ? "[Notion Flow] 上传视频文件失败:"
                          : "[Notion Flow] Upload video file error:",
                        e
                      )
                      return Promise.reject(null)
                    }
...
const notionMeta2string = (meta: Meta): Promise<string> => {
  ...
  return storage.get<PublisherOptions>("options").then((props) => {
    ...
    if (frontMatter) {
      try {
        ...
        logToRenderer(
          "error",
          cn
            ? "[Notion Flow] Front Matter 配置错误, 忽略之:"
            : "[Flow] Front Matter config error, it will be ignored:",
          e
        )
      }
    }
    ...
    const dateString = `${_date.getFullYear()}-${
      _date.getMonth() + 1
    }-${_date.getDate()} ${_date.getHours()}:${_date.getMinutes()}:00 +0800`
    let _lastUpdateTime = ""
    if (publisher.autoAddLastUpdateTime) {
      logToRenderer(
        "info",
        cn
          ? "[Github] 添加 「lastUpdateTime」 Front Matter"
          : "[Github] Will add 「lastUpdateTime」 frontmatter"
      )
      const _ = new Date()
      _lastUpdateTime = `${_.getFullYear()}-${
        _.getMonth() + 1
      }-${_.getDate()} ${_.getHours()}:${_.getMinutes()}:00 +0800`
    }
    return `---
title: ${title}
date: ${dateString}${
...
const getISODateTime = (time: Date): string => {
  ...
  return `${year}-${normalizeNum(month)}-${normalizeNum(date)}T${normalizeNum(
    hour
  )}:${normalizeNum(minute)}:${normalizeNum(second)}.000+08:00`
}
