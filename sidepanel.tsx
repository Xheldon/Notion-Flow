...
import store, { setLogs, setToc } from "$store"
import type { PublisherOptions, State } from "$types"
import { _toContent, getPublisherConfig } from "$utils"
import {
  Button,
  Collapse,
  ConfigProvider,
          ...
          <Tabs
            ...
            tabBarExtraContent={{
              right: (
                <>
                  <Tooltip
                    title={cn ? "Notion 返回顶部" : "Notion Back to top"}>
            ...
            renderTabBar={(props, DefaultBar) => {
              return (
                <StickyBox
                  offsetTop={0}
                  style={{ zIndex: 999, background: "rgb(251, 251, 250)" }}>
                  <DefaultBar {...props} />
                </StickyBox>
              )
            }}
